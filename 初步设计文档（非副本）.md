- ## 系统架构
  ...
  *（可参考图“一致性哈希.png”，其中key即为repo，peer即为worker）*
  ...
  【连接好一定量worker后再分发任务（**速度**？） VS 每连接一个worker就hash分发一部分repo（存在**抖动问题**）】
  
  - 方案1（v）：最初启动时先启动好一定量的worker，等全部启动好后再进行分发【？启动100个worker需要多久？（是否影响coordinator“快速启动”的需求）】
  
    “启动好”的标准：记录平均超时时长，超时后worker数即定。想要添加worker，统一coordinator调用接口添加
  ...
  - 方案2：先定环上peer数，并提前算好repo和worker之间的映射【万一指定100个worker但是有worker挂了怎么办】
    ...
    开始监听，每和一个worker连接-> w9orker_conn++，然后开始通过map查找
