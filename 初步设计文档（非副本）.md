- ## 系统架构
  ...
  1. **Coordinator与Worker之间：**
     ...
     基于低延迟、高性能要求，可使用轻量级**RPC**来实现通信。

     

     > 首先coordinator启动等待10min，到时间有一定数量得worker连接之后固定哈希，确定好哈希环结构之后coordinator开始计算每一个worker与repo之间的哈希对应，并且记录下来。
     >
     > 之后worker开始逐个向coordinator发送rpc请求`getURL`，request是worker的唯一ID，获得的response是自己所需要负责的repoURL
     >
     > 心跳机制的设计主要是worker定期发送心跳包给coordinator，由coordinator去累计一个接收到的心跳包sum，然后每次通过这个总和去判断哪个worker已经很久没有发送心跳包了（可以设置一个最大限度的查值）
     >
