- ## 系统架构
  ...
  *（可参考图“一致性哈希.png”，其中key即为repo，peer即为worker）*
  ...
  `get_repo_priority`：repo is clone? -> (no) default **step**=STEP_SECONDS *(clone_fail_cnt + 1)
...
2、workers	# also includes aggregator
...
自己维护repo的元数据（struct/本地日志文件）（主要为了优先级pri服务，不需要持久化）
