...
？【worker如何知道哪个url是自己维护的-> connect send / belong_worker】
...
- #### coordinator
  ...
  read config_file -> ( git clone/fetch/update urls_git_repo ) 
  read all_urls、create data structure （buf）in memory、read db

  ```c
  // 1read1write
  func read_repo_files_into_database{
      ...
      for (file!= null) {
  		read(file,buf,size);  // read 1 url_repo into memory
  		file++;  // next file
          git_repo = buf[0];
          repo_url = buf[1];
          if (buf[2] != null) {
              // 【?】urls_git_repo格式，是否存在、如何处理
              repo_url2 = buf[2];
          }
          // add into database
  		es_client.index(index:'list', id:git_repo, body:repo_url);
  	}
      ...
  }
  ```
